...
func (s *SaveArticleImages) ArticleCreateBefore(item *model.Article) (err error) {
	if !s.EnableOnCreate {
		return nil
	}
	return s.Save(item)
}
func (s *SaveArticleImages) ArticleUpdateBefore(item *model.Article) (err error) {
	if !s.EnableOnUpdate {
		return nil
	}
	return s.Save(item)
...
func (s *SaveArticleImages) Save(item *model.Article) error {
	s.initDownReferer()
...
func (s *SaveArticleImages) eachSave(item *model.Article) func(i int, sn *goquery.Selection) {
...
func (s *SaveArticleImages) logInfo(item *model.Article, src string, err error) []zap.Field {
	return []zap.Field{zap.String("url", src), zap.String("title", item.Title), zap.Error(err)}
}

// 上传缩略图
func (s *SaveArticleImages) uploadThumbnail(item *model.Article, file []byte, name, ext, imgType string) (err error) {
...
func (s *SaveArticleImages) down(item *model.Article, uri string) (file []byte, err error) {
...
func (s *SaveArticleImages) saveThumbnail(item *model.Article) {
	if item.Thumbnail == "" {
		return
	}
	// 判断是否是当前的上传域
