...
func (p *PushToBaidu) ArticleCreateAfter(item *model.Article) {
	if p.EnableOnCreate {
		p.pushArticle(item, "create")
	}
}

func (p *PushToBaidu) ArticleUpdateAfter(item *model.Article) {
	if p.EnableOnUpdate {
		p.pushArticle(item, "update")
	}
}

func (p *PushToBaidu) pushArticle(item *model.Article, action string) {
