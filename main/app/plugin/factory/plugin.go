	...
	return zap.New(
		zapcore.NewCore(zapcore.NewJSONEncoder(zapcore.EncoderConfig{
			...
			zapcore.NewMultiWriteSyncer(zapcore.AddSync(&lumberjack.Logger{
				...
				MaxSize:    config.Set.Log.Plugin.MaxSize,
				MaxAge:     config.Set.Log.Plugin.MaxAge,
				MaxBackups: config.Set.Log.Plugin.MaxBackups,
				Compress:   config.Set.Log.Plugin.Compress,
			})),
